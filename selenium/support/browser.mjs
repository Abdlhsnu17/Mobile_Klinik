...
// Mengambil log console browser level SEVERE (error JS tak tertangani, request
// gagal, dst.) sejak terakhir dipanggil. Berguna untuk mendiagnosis kegagalan
// skenario yang tidak jelas penyebabnya dari sisi UI saja.
export async function getSevereBrowserLogs(driver) {
  try {
    const entries = await driver.manage().logs().get(logging.Type.BROWSER);
    return entries.map((entry) => `[${entry.level.name}] ${entry.message}`);
  } catch {
    return [];
  }
}
