...
describe("ProcurementService.receivePurchaseOrder", () => {
    beforeEach(() => {
        jest.resetAllMocks();
        mockedMedicineService.receiveBatch.mockResolvedValue({});
        mockedCollectionService.updateItem.mockImplementation(async (_c, _id, patch) => ({ ...basePo, ...patch }));
    });
    it("menambah stok via receiveBatch dan menandai status selesai bila semua diterima penuh", async () => {
        mockedCollectionService.findById.mockResolvedValue(structuredClone(basePo));
        const result = await procurementService_1.ProcurementService.receivePurchaseOrder("po1", [
        ...
        expect(mockedMedicineService.receiveBatch).toHaveBeenCalledWith(expect.objectContaining({ medicineId: "m1", quantity: 100, batchNumber: "B1", supplier: "PT Sehat" }));
    ...
    it("menandai status diterima-sebagian bila hanya sebagian diterima", async () => {
        ...
        expect(savedItems.find((i) => i.medicineId === "m1").receivedQuantity).toBe(40);
    });
    it("menolak jumlah terima melebihi sisa pesanan", async () => {
        mockedCollectionService.findById.mockResolvedValue(structuredClone(basePo));
        await expect(procurementService_1.ProcurementService.receivePurchaseOrder("po1", [
    ...
    it("menolak bila PO tidak ditemukan", async () => {
        ...
        await expect(procurementService_1.ProcurementService.receivePurchaseOrder("x", [{ medicineId: "m1", receivedQuantity: 1, batchNumber: "B", expiryDate: "2027-01-01" }])).rejects.toMatchObject({ statusCode: 404 });
    });
    it("menolak menerima PO yang sudah selesai", async () => {
        mockedCollectionService.findById.mockResolvedValue({ ...structuredClone(basePo), status: "selesai" });
        await expect(procurementService_1.ProcurementService.receivePurchaseOrder("po1", [{ medicineId: "m1", receivedQuantity: 1, batchNumber: "B", expiryDate: "2027-01-01" }])).rejects.toMatchObject({ statusCode: 409 });
    });
...
//# sourceMappingURL=procurementService.test.js.map