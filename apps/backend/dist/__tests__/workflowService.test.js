...
describe("WorkflowService alur pemeriksaan", () => {
    ...
    it("membuat antrean awal tanpa dokter dan layanan", async () => {
        ...
        await workflowService_1.WorkflowService.registerVisit({
            patientId: "patient-1",
            date: "2026-07-19",
            time: "08:00",
        });
        expect(mockedCollectionService.createItem).toHaveBeenCalledWith("appointments", expect.objectContaining({
            patientId: "patient-1",
            doctorId: "",
    ...
    it("hanya memulai pemeriksaan dari status Dipanggil", async () => {
        ...
        await expect(workflowService_1.WorkflowService.startExam("appt-1")).resolves.toMatchObject({
            status: "Diperiksa",
        });
        expect(mockedCollectionService.updateItem).toHaveBeenCalledWith("appointments", "appt-1", { status: "Diperiksa" });
    });
    it.each(["Menunggu", "Diperiksa", "Selesai", "Batal"])("menolak mulai pemeriksaan dari status %s", async (status) => {
        mockedCollectionService.findById.mockResolvedValue({
            id: "appt-1",
            status,
        });
        await expect(workflowService_1.WorkflowService.startExam("appt-1")).rejects.toMatchObject({
            statusCode: 409,
        });
    ...
    it("menolak simpan pemeriksaan bila status belum Diperiksa", async () => {
        ...
        await expect(workflowService_1.WorkflowService.finishExam({
