...
function mockBillingCollections(payments = [], existingBillings = []) {
    mockedCollectionService.list.mockImplementation(async (collection) => {
        if (collection === "patients")
            return [{ id: "patient-1", name: "Budi" }];
        ...
        if (collection === "payments")
            return payments;
        if (collection === "insuranceProfiles")
            return [];
        if (collection === "billingRecords")
            return existingBillings;
...
describe("BillingService.recordPayment", () => {
    beforeEach(() => {
        ...
        mockedCollectionService.findById.mockImplementation(async (collection, id) => {
            if (collection === "medicalRecords" && id === medicalRecord.id)
                return medicalRecord;
            return null;
        });
    });
    it("mencatat pembayaran hanya sebesar sisa tagihan saat nominal diterima lebih besar", async () => {
        ...
        mockedCollectionService.createItem.mockImplementation(async (collection, payload) => {
            if (collection === "billingRecords")
                return { id: "bill-1", ...payload };
            if (collection === "payments")
                return { id: "pay-1", ...payload };
            return { id: "item-1", ...payload };
        });
        mockedCollectionService.updateItem.mockResolvedValue({ id: "bill-1" });
        const payment = await billingService_1.BillingService.recordPayment({
            patientId: "patient-1",
        ...
        expect(mockedCollectionService.createItem).toHaveBeenCalledWith("billingRecords", expect.objectContaining({
            serviceCost: 100000,
            medicineCost: 24000,
            labCost: 50000,
            total: 174000,
        ...
        expect(mockedCollectionService.createItem).toHaveBeenCalledWith("payments", expect.objectContaining({
            patientId: "patient-1",
            medicalRecordId: "mr-1",
            amount: 149000,
        }));
    ...
    it("menolak pembayaran jika pasien tidak sesuai dengan rekam medis", async () => {
        ...
        await expect(billingService_1.BillingService.recordPayment({
...
describe("BillingService.updatePayment", () => {
    ...
    it("membatasi nominal update pembayaran sampai sisa slot tagihan", async () => {
        ...
        mockedCollectionService.updateItem.mockImplementation(async (collection, id, payload) => {
            if (collection === "payments")
                return { id, ...payload };
            return { id, ...payload };
        });
        const updated = await billingService_1.BillingService.updatePayment("pay-1", {
            amount: 150000,
        });
        expect(updated?.amount).toBe(150000);
        expect(mockedCollectionService.updateItem).toHaveBeenCalledWith("payments", "pay-1", expect.objectContaining({
            amount: 150000,
            patientId: "patient-1",
            medicalRecordId: "mr-1",
        }));
