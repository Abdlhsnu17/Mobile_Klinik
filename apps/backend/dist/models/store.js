...
async function withTransaction(fn) {
    const conn = await mysqlClient_1.default.getConnection();
    try {
...
const REQUIRED_DEFAULT_USERS = [defaultData_1.DEFAULT_SUPER_ADMIN_USER, defaultData_1.DEFAULT_ADMIN_USER];
const tableMap = {
    users: "users",
    patients: "patients",
    doctors: "doctors",
...
async function columnExists(table, column) {
    const [rows] = await mysqlClient_1.default.query("SELECT COUNT(*) as total FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = ? AND COLUMN_NAME = ?", [table, column]);
    return (rows[0]?.total ?? 0) > 0;
}
async function ensureDatabaseSchema() {
    for (const statement of createTableStatements) {
        await mysqlClient_1.default.query(statement);
    }
    for (const [table, columns] of Object.entries(requiredColumns)) {
        for (const [column, definition] of Object.entries(columns)) {
            if (!(await columnExists(table, column))) {
                await mysqlClient_1.default.query(`ALTER TABLE \`${table}\` ADD COLUMN \`${column}\` ${definition}`);
            }
        }
    }
}
...
function pickColumns(collection, item) {
    const picked = {};
...
async function insertDetails(executor, collection, item) {
    ...
    if (collection === "appointments") {
        ...
        if (item.triage) {
            await executor.query(`INSERT INTO appointment_triage (
    ...
    if (collection === "medicalRecords") {
        for (const prescription of item.prescription ?? []) {
            await executor.query("INSERT INTO prescriptions (id, medicalRecordId, medicineId, medicineName, dosage, frequency, duration, quantity, notes) VALUES (UUID(), ?, ?, ?, ?, ?, ?, ?, ?)", [item.id, prescription.medicineId, prescription.medicineName, prescription.dosage, prescription.frequency, prescription.duration, prescription.quantity, prescription.notes ?? null]);
        }
        for (const equipment of item.equipmentsUsed ?? []) {
            await executor.query("INSERT INTO medical_record_equipments (id, medicalRecordId, equipmentId, equipmentName, usageNotes) VALUES (UUID(), ?, ?, ?, ?)", [item.id, equipment.equipmentId, equipment.equipmentName, equipment.usageNotes ?? null]);
        }
        if (item.vitalSigns) {
            await executor.query("INSERT INTO vital_signs (medicalRecordId, bloodPressure, heartRate, temperature, bloodGlucose, oxygenSaturation, weight, height, respiratoryRate) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", [item.id, item.vitalSigns.bloodPressure ?? null, item.vitalSigns.heartRate ?? null, item.vitalSigns.temperature ?? null, item.vitalSigns.bloodGlucose ?? null, item.vitalSigns.oxygenSaturation ?? null, item.vitalSigns.weight ?? null, item.vitalSigns.height ?? null, item.vitalSigns.respiratoryRate ?? null]);
        }
    }
    ...
    if (collection === "pharmacyRequests") {
        for (const prescription of item.items ?? item.prescription ?? []) {
            await executor.query("INSERT INTO pharmacy_request_items (id, pharmacyRequestId, medicineId, medicineName, dosage, frequency, duration, quantity, notes) VALUES (UUID(), ?, ?, ?, ?, ?, ?, ?, ?)", [item.id, prescription.medicineId, prescription.medicineName, prescription.dosage, prescription.frequency, prescription.duration, prescription.quantity, prescription.notes ?? null]);
        }
    }
}
async function deleteDetails(executor, collection, id) {
    const detailNames = detailTables[collection] ?? [];
    for (const table of detailNames) {
        const fk = table === "doctor_schedules" ? "doctorId" :
...
async function hydrateDetails(collection, rows) {
    if (rows.length === 0)
        return rows;
    if (collection === "doctors") {
        const [details] = await mysqlClient_1.default.query("SELECT doctorId, day, startTime, endTime FROM doctor_schedules");
        return rows.map((row) => ({ ...row, schedules: details.filter((item) => item.doctorId === row.id).map(({ doctorId, ...schedule }) => schedule) }));
    }
    if (collection === "appointments") {
        const [details] = await mysqlClient_1.default.query("SELECT appointmentId, serviceId FROM appointment_services");
        const [triages] = await mysqlClient_1.default.query("SELECT * FROM appointment_triage");
        return rows.map((row) => {
            const serviceIds = details.filter((item) => item.appointmentId === row.id).map((item) => item.serviceId);
            const triage = triages.find((item) => item.appointmentId === row.id);
            return {
                ...row,
    ...
    if (collection === "medicalRecords") {
        const [prescriptions] = await mysqlClient_1.default.query("SELECT medicalRecordId, medicineId, medicineName, dosage, frequency, duration, quantity, notes FROM prescriptions");
        const [equipments] = await mysqlClient_1.default.query("SELECT medicalRecordId, equipmentId, equipmentName, usageNotes FROM medical_record_equipments");
        const [vitals] = await mysqlClient_1.default.query("SELECT * FROM vital_signs");
        return rows.map((row) => ({
            ...row,
            prescription: prescriptions.filter((item) => item.medicalRecordId === row.id).map(({ medicalRecordId, ...item }) => normalizeRow(item)),
            equipmentsUsed: equipments.filter((item) => item.medicalRecordId === row.id).map(({ medicalRecordId, ...item }) => normalizeRow(item)),
            vitalSigns: normalizeRow(vitals.find((item) => item.medicalRecordId === row.id) ?? {}),
        }));
    }
    if (collection === "labOrders") {
        const [tests] = await mysqlClient_1.default.query("SELECT labOrderId, testName FROM lab_order_tests");
        return rows.map((row) => ({ ...row, tests: tests.filter((item) => item.labOrderId === row.id).map((item) => item.testName) }));
    }
    if (collection === "pharmacyRequests") {
        const [items] = await mysqlClient_1.default.query("SELECT pharmacyRequestId, medicineId, medicineName, dosage, frequency, duration, quantity, notes FROM pharmacy_request_items");
        return rows.map((row) => {
            const prescriptions = items.filter((item) => item.pharmacyRequestId === row.id).map(({ pharmacyRequestId, ...item }) => normalizeRow(item));
    ...
    if (collection === "auditLogs") {
        return rows.map((row) => {
            ...
            if (normalized.beforeSnapshot)
                normalized.before = JSON.parse(normalized.beforeSnapshot);
            if (normalized.afterSnapshot)
                normalized.after = JSON.parse(normalized.afterSnapshot);
...
async function initDataStore() {
    ...
    for (const user of REQUIRED_DEFAULT_USERS) {
        await mysqlClient_1.default.query(`INSERT INTO users (id, username, name, email, role, password, createdAt)
       VALUES (?, ?, ?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE
         username = VALUES(username),
         name = VALUES(name),
         email = VALUES(email),
         role = VALUES(role)`, [user.id, user.username, user.name, user.email, user.role, user.password, toMysqlDatetime(user.createdAt)]);
    }
    for (const collection of Object.keys(defaultData_1.defaultData)) {
        if (collection === "users")
            continue;
        const [rows] = await mysqlClient_1.default.query(`SELECT COUNT(*) as total FROM \`${tableMap[collection]}\``);
        if ((rows[0]?.total ?? 0) === 0 && defaultData_1.defaultData[collection].length > 0) {
            await writeCollection(collection, defaultData_1.defaultData[collection]);
        }
    }
}
async function readCollection(collection) {
    const [rows] = await mysqlClient_1.default.query(`SELECT * FROM \`${tableMap[collection]}\``);
...
async function updateOne(collection, id, item) {
    await withTransaction(async (conn) => {
        const data = pickColumns(collection, item);
        delete data.id;
