...
class ReportService {
    static async getMorbidity(top = 10) {
    ...
    static async getVisits() {
        const appointments = await collectionService_1.CollectionService.list("appointments");
        ...
        const monthlyTrend = Array.from({ length: 6 }, (_, offset) => {
            ...
            const count = appointments.filter((appt) => {
                ...
                return (apptDate.getFullYear() === month.getFullYear() &&
    ...
    static async getFinancials() {
        const payments = await collectionService_1.CollectionService.list("payments");
        const billingRecords = await collectionService_1.CollectionService.list("billingRecords");
        const records = await collectionService_1.CollectionService.list("medicalRecords");
        const appointments = await collectionService_1.CollectionService.list("appointments");
        const doctors = await collectionService_1.CollectionService.list("doctors");
        ...
        const insuranceClaims = billingRecords.reduce((sum, billing) => sum + (billing.status === "claimed_to_insurance" ? Math.max(0, Number(billing.total ?? 0) - Number(billing.paidAmount ?? 0)) : 0), 0);
        const byBillingStatus = Object.entries(billingRecords.reduce((acc, billing) => {
        ...
        const paymentTotalByRecord = new Map();
        payments.forEach((payment) => {
            paymentTotalByRecord.set(payment.medicalRecordId, (paymentTotalByRecord.get(payment.medicalRecordId) ?? 0) + Number(payment.amount ?? 0));
        });
        const billingMap = new Map(billingRecords.map((billing) => [billing.medicalRecordId, billing]));
        const byDoctor = doctors.map((doctor) => {
            ...
            const revenue = doctorAppointments.reduce((sum, appt) => {
                ...
                if (billing)
                    return sum + Number(billing.paidAmount ?? 0);
    ...
    static async getPaymentSummaries() {
        ...
        const appointments = await collectionService_1.CollectionService.list("appointments");
        const services = await collectionService_1.CollectionService.list("services");
        const medicines = await collectionService_1.CollectionService.list("medicines");
        const patients = await collectionService_1.CollectionService.list("patients");
    ...
    static async getReferralStats(filters) {
        let referrals = await collectionService_1.CollectionService.list("referrals");
        if (filters?.from)
            referrals = referrals.filter((referral) => referral.createdAt >= filters.from);
        if (filters?.to)
            referrals = referrals.filter((referral) => referral.createdAt <= filters.to);
        if (filters?.facilityId)
            referrals = referrals.filter((referral) => referral.facilityId === filters.facilityId);
        const byStatus = Object.entries(referrals.reduce((acc, referral) => {
            acc[referral.status] = (acc[referral.status] ?? 0) + 1;
            return acc;
        }, {})).map(([status, total]) => ({ status, total }));
        const byDirection = Object.entries(referrals.reduce((acc, referral) => {
            acc[referral.direction] = (acc[referral.direction] ?? 0) + 1;
            return acc;
        }, {})).map(([direction, total]) => ({ direction, total }));
        const byFacility = Object.entries(referrals.reduce((acc, referral) => {
    ...
    static async getProfitLoss(filters) {
        ...
        const totalRevenue = payments
            .filter((payment) => inRange(payment.paidAt ?? payment.createdAt))
