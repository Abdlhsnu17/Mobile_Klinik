...
class InformedConsentPdfService {
    static async buildConsentPdf(consentId) {
        const consent = await collectionService_1.CollectionService.findById("informedConsents", consentId);
        if (!consent) {
            throw (0, httpError_1.createHttpError)(404, "Persetujuan tindakan tidak ditemukan.");
        }
        ...
        if (consent.procedureCode)
            field("Kode Tindakan (ICD-9-CM)", consent.procedureCode);
        ...
        const giver = consent.grantedBy === "wali"
        ...
        doc.moveDown(0.3);
        doc.font("Helvetica").fontSize(10).text(`Saya yang bertanda tangan di bawah ini, ${giver}, setelah mendapatkan penjelasan yang cukup mengenai tindakan di atas beserta tujuan, risiko, dan alternatifnya, dengan ini menyatakan `, { continued: true, align: "justify" });
...
exports.InformedConsentPdfService = InformedConsentPdfService;
//# sourceMappingURL=informedConsentPdfService.js.map