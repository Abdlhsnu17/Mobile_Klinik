...
export class ReferralPdfService {
  static async buildReferralLetterPdf(referralId: string) {
    ...
    if (referral.direction !== "outgoing") {
      throw createHttpError(400, "Surat rujukan hanya tersedia untuk rujukan keluar.")
    }
    ...
    if (referral.sentAt) field("Tanggal Dikirim", new Date(referral.sentAt).toLocaleDateString("id-ID"))

    doc.moveDown(2)
    doc.font("Helvetica").fontSize(11).text(
      "Bersama surat ini kami mohon kesediaan Bapak/Ibu untuk menerima dan menindaklanjuti penanganan pasien tersebut di atas. Atas kerja sama yang baik, kami ucapkan terima kasih.",
      { align: "justify" }
    )
