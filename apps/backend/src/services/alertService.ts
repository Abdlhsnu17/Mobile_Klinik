...
function daysUntil(dateValue: string | undefined, today: Date): number | null {
...
function isActiveMedicine(medicine: Medicine): boolean {
  // Kolom isActive tersimpan sebagai tinyint (0/1) atau boolean; anggap aktif bila
  // tidak eksplisit dimatikan (0 / false).
  const flag: unknown = (medicine as { isActive?: unknown }).isActive
  return flag !== false && flag !== 0
}

function buildLowStockAlerts(medicines: Medicine[]): OperationalAlert[] {
  const alerts: OperationalAlert[] = []
  for (const medicine of medicines) {
    if (!isActiveMedicine(medicine)) continue
...
function buildExpiryAlerts(medicines: Medicine[], today: Date, expiryDays: number): OperationalAlert[] {
  const alerts: OperationalAlert[] = []
  for (const medicine of medicines) {
    if (!isActiveMedicine(medicine)) continue
    const days = daysUntil(medicine.expiryDate, today)
    if (days === null || days > expiryDays) continue
...
function buildMaintenanceAlerts(
  ...
  for (const equipment of equipments) {
    if (equipment.status === "Tidak Aktif") continue
    const days = daysUntil(equipment.nextMaintenanceDate, today)
    if (days === null || days > maintenanceDays) continue
...
const SEVERITY_RANK: Record<OperationalAlert["severity"], number> = { critical: 0, warning: 1 }
...
export class AlertService {
  static async getOperationalAlerts(options?: {
    ...
    const byCategory = alerts.reduce(
      ...
      { "stok-menipis": 0, "obat-kadaluarsa": 0, "maintenance-alat": 0 } as Record<AlertCategory, number>,
    )
